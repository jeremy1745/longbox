...
// StatusBreakdown returns counts of backlog_items grouped by status across all
// non-paused runs. Used by the pipeline-status endpoint to surface why nothing
// is happening (everything exhausted? all pending but blocked? nothing queued?).
func (r *BacklogRepo) StatusBreakdown() (map[string]int, error) {
	rows, err := r.read.Query(`
		SELECT bi.status, COUNT(*)
		FROM backlog_items bi
		JOIN backlog_runs br ON br.id = bi.backlog_run_id
		WHERE br.paused = 0
		GROUP BY bi.status
	`)
	if err != nil {
		return nil, fmt.Errorf("backlog status breakdown: %w", err)
	}
	defer rows.Close()
...
// ReconcileOwned walks every non-terminal backlog item, checks whether the
// underlying issue now has a file or a completed download, and marks those
// items 'completed' instead of leaving them stuck on 'failed' / 'pending'.
// Returns the number of items reconciled. Fixes the symptom where backlog
// items pile up as "failed (no nzb found)" because the issue was already
// downloaded earlier and AutoSearchAndGrab returns nil for already-grabbed
// issues.
func (r *BacklogRepo) ReconcileOwned() (int64, error) {
	res, err := r.write.Exec(`
		...
		WHERE status NOT IN ('completed','cancelled')
		  AND issue_id IN (
		    ...
		    LEFT JOIN download_history dh ON dh.issue_id = i.id AND dh.status = 'completed'
		    WHERE cf.id IS NOT NULL OR dh.id IS NOT NULL
		  )
	`)
	if err != nil {
		return 0, fmt.Errorf("reconciling owned backlog items: %w", err)
	}
	n, _ := res.RowsAffected()
	return n, nil
}

// IssueHasFileOrGrab reports whether the given issue is already represented
// by a comic file on disk or a completed download row. The queue worker uses
// this as a pre-check to avoid burning a search slot on already-owned issues.
func (r *BacklogRepo) IssueHasFileOrGrab(issueID int64) (bool, error) {
	var n int
	err := r.read.QueryRow(`
		SELECT
		  (SELECT COUNT(*) FROM comic_files WHERE issue_id = ?) +
		  (SELECT COUNT(*) FROM download_history WHERE issue_id = ? AND status = 'completed')
...
// LastFailures returns the most recent N failed items with their last_error.
// Useful for surfacing what's actually breaking when a pipeline stalls.
func (r *BacklogRepo) LastFailures(limit int) ([]model.BacklogItem, error) {
	if limit <= 0 || limit > 100 {
		limit = 10
	}
	rows, err := r.read.Query(`
		SELECT bi.id, bi.backlog_run_id, bi.issue_id,
			COALESCE(i.issue_number,''), COALESCE(se.title,''),
			bi.status, COALESCE(bi.last_error,''), bi.retry_count, bi.updated_at
		FROM backlog_items bi
		JOIN backlog_runs br ON br.id = bi.backlog_run_id
		LEFT JOIN issues i ON i.id = bi.issue_id
		LEFT JOIN series se ON se.id = bi.series_id
		WHERE br.paused = 0 AND bi.status IN ('failed','error')
		ORDER BY bi.updated_at DESC
		LIMIT ?
	`, limit)
	if err != nil {
		return nil, fmt.Errorf("last failures: %w", err)
	}
	defer rows.Close()
	var out []model.BacklogItem
	for rows.Next() {
		var it model.BacklogItem
		var updatedAt string
		if err := rows.Scan(&it.ID, &it.BacklogRunID, &it.IssueID, &it.IssueNumber, &it.SeriesTitle,
			&it.Status, &it.LastError, &it.RetryCount, &updatedAt); err != nil {
			return nil, err
		}
...
// RetryAllInRun resets every failed or errored item in a run back to pending,
// clearing retry timers and download IDs. Returns the number of items affected.
func (r *BacklogRepo) RetryAllInRun(runID int64) (int64, error) {
	res, err := r.write.Exec(`
		UPDATE backlog_items
		SET status = 'pending', retry_at = NULL, last_error = '',
			download_history_id = NULL, sab_nzo_id = '', nzb_guid = '',
			updated_at = (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
		WHERE backlog_run_id = ? AND status IN ('failed','error')
	`, runID)
	if err != nil {
		return 0, fmt.Errorf("retry all in run: %w", err)
	}
...
// ClaimNextCandidate atomically marks the next eligible backlog item as
// 'searching' and returns it. Used by concurrent queue workers — without
// the atomic claim, two workers can read the same row from
// FindNextCandidate before either writes a new status, causing a
// double-grab. Implementation uses SQLite's UPDATE … RETURNING (3.35+) so
// the read+write happens in one statement under the write lock.
//
// Returns (nil, nil) when no eligible item exists.
func (r *BacklogRepo) ClaimNextCandidate(maxRetries int, now time.Time) (*model.BacklogItem, error) {
	row := r.write.QueryRow(`
		UPDATE backlog_items
		SET status = 'searching',
		    ...
		    updated_at = (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
		WHERE id = (
		    SELECT bi.id FROM backlog_items bi
		    JOIN backlog_runs br ON br.id = bi.backlog_run_id
		    WHERE br.paused = 0
		      AND (bi.status = 'pending'
		           OR (bi.status = 'failed'
		               AND (bi.retry_at IS NULL OR bi.retry_at = '' OR bi.retry_at <= ?)
		               AND (? <= 0 OR bi.retry_count < ?)))
		    ORDER BY CASE bi.status WHEN 'pending' THEN 0 ELSE 1 END, bi.created_at ASC
		    LIMIT 1
		)
		RETURNING id, backlog_run_id, series_id, issue_id,
		          COALESCE(variant_name,''), priority, status,
		          retry_count, COALESCE(retry_at,''), COALESCE(last_error,''),
		          COALESCE(sab_nzo_id,''), COALESCE(nzb_guid,''),
		          download_history_id, created_at, updated_at
	`, now.UTC().Format(time.RFC3339), maxRetries, maxRetries)
	var it model.BacklogItem
	var dlHistoryID sql.NullInt64
	if err := row.Scan(&it.ID, &it.BacklogRunID, &it.SeriesID, &it.IssueID,
		&it.VariantName, &it.Priority, &it.Status,
		&it.RetryCount, &it.RetryAt, &it.LastError,
		&it.SabNzoID, &it.NZBGuid,
		&dlHistoryID, &it.CreatedAt, &it.UpdatedAt); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
		return nil, fmt.Errorf("claim next candidate: %w", err)
	}
	if dlHistoryID.Valid {
		it.DownloadHistoryID = &dlHistoryID.Int64
	}
	// Backfill display fields the worker uses for activity events.
	if err := r.read.QueryRow(`
		SELECT COALESCE(i.issue_number,''), COALESCE(se.title,'')
		FROM backlog_items bi
		LEFT JOIN issues i ON i.id = bi.issue_id
		LEFT JOIN series se ON se.id = bi.series_id
		WHERE bi.id = ?
	`, it.ID).Scan(&it.IssueNumber, &it.SeriesTitle); err != nil && err != sql.ErrNoRows {
		return nil, fmt.Errorf("fetching display fields: %w", err)
	}
	return &it, nil
}
...
// RelinkToExistingDownload reattaches a backlog item to an in-flight or
// already-completed download_history row for the same issue. Used when
// AutoSearchAndGrab returns Reason="already grabbed" — a prior cycle's
// download_history row exists but the backlog item's download_history_id
// got cleared (typically by RetryItem). Without re-linking, the dl status
// callback in BacklogService.HandleDownloadStatus can't find the backlog
// item by FindByDownloadHistory and the item never reaches a terminal
// state. Returns true when a link was established.
func (r *BacklogRepo) RelinkToExistingDownload(itemID, issueID int64) (bool, error) {
	var dlID int64
	var status, sabID, nzbGuid string
	err := r.read.QueryRow(`
		SELECT id, status, COALESCE(external_id,''), COALESCE(nzb_guid,'')
		FROM download_history
		WHERE issue_id = ? AND status IN ('grabbed','downloading','completed')
		ORDER BY id DESC
		LIMIT 1
	`, issueID).Scan(&dlID, &status, &sabID, &nzbGuid)
	if err != nil {
		if err == sql.ErrNoRows {
			return false, nil
		}
		return false, fmt.Errorf("looking up existing dl_history: %w", err)
	}
	// Map dl status to backlog item status. Completed dl → completed
	// backlog. In-flight dl → queued, so the user sees it active.
	itemStatus := "queued"
	if status == "completed" {
	...
	_, err = r.write.Exec(`
		UPDATE backlog_items
		SET download_history_id = ?, sab_nzo_id = ?, nzb_guid = ?, status = ?,
		    last_error = '', retry_at = NULL,
		    updated_at = (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
		WHERE id = ?
	`, dlID, sabID, nzbGuid, itemStatus, itemID)
	if err != nil {
		return false, fmt.Errorf("relinking backlog item: %w", err)
	}
	return true, nil
}
...
// FindByID returns the backlog item with the given primary key, or nil if
// the row no longer exists.
func (r *BacklogRepo) FindByID(id int64) (*model.BacklogItem, error) {
	row := r.read.QueryRow(backlogItemSelect+` WHERE bi.id = ? LIMIT 1`, id)
	return scanBacklogItem(row)
}
