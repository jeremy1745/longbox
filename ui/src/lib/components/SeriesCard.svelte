...
<div class="group relative rounded-lg overflow-hidden bg-gray-800 shadow-lg hover:shadow-xl hover:ring-2 hover:ring-amber-400/50 transition-all">
	<a
	...
	<!--
		Delete button — sibling of the anchor, NOT a descendant. Nesting it
		inside the <a> made click events bubble through SvelteKit's link
		interceptor and navigate despite preventDefault. As a sibling at z-20
		on the parent group, the click target is unambiguously the button and
		the anchor never sees the event.
	-->
	{#if onDelete}
		<button
			type="button"
			onclick={handleDelete}
			class="absolute {series.tracked ? 'top-9' : 'top-2'} right-2 w-6 h-6 rounded-full bg-red-600/90 hover:bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg z-20"
			title="Delete series"
